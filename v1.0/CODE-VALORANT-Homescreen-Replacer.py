...
try:
    ...
    # 只是拖拽功能不可用
    # 考虑是否不对无 thkinterdnd2 的用户显示拖拽功能
    TkinterDnD = None
...
APP_NAME = "VALORANT 主页面背景视频替换器" # riot 你赶紧给主页改回去啊。。要不然我这工具都没啥用:)
...
def search_valorant_paths(): # 就是下面这个第一条的 Riot Games 你的美术其实挺好的为啥要一打开就是房间，多加一个主页面呗。无语子。。，，，
    """
    搜索所有磁盘：
